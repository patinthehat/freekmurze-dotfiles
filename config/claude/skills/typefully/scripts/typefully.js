...
function getDefaultSocialSetId() {
    ...
    if (globalConfig?.defaultSocialSetId) {
        return {
            ...
            id: globalConfig.defaultSocialSetId,
        };
    }
    ...
    if (!response.ok) {
        if (exitOnError) {
            ...
            if (
                ...
                error(`Validation error: ${validationMessage}`, {
                    response: data,
                });
            }
        ...
        if (arg.startsWith('--')) {
            ...
            if (spec[key] === 'boolean') {
                ...
                if (rawKey === 'social-set-id' || rawKey === 'social_set_id') {
                    error(
                        '--social-set-id (or --social_set_id) requires a value',
                    );
                }
...
function getSocialSetIdFromParsed(parsed) {
    ...
    if (value.trim() === '') {
        error(
            '--social-set-id (or --social_set_id) requires a non-empty value',
        );
    }
    return value;
}
...
async function cmdSetup(args) {
    ...
    if (!apiKey) {
        ...
        console.error(
            fmt.dim(
                "Sign up free at typefully.com if you don't have an account.",
            ),
        );
        console.error('');
        console.error(
            fmt.info(`Get your API key at: ${fmt.link(API_KEY_URL)}`),
        );
        console.error('');
        apiKey = await prompt(
        ...
        if (fs.existsSync(gitignorePath)) {
            ...
            if (
                ...
                if (isNonInteractive) {
                    ...
                    console.error(
                        fmt.success('Added .typefully/ to .gitignore'),
                    );
                } else {
                    console.error('');
            ...
            if (isNonInteractive) {
                ...
                if (createGitignore.toLowerCase() !== 'n') {
                    ...
                    console.error(
                        fmt.success(
                            'Created .gitignore with .typefully/ entry',
                        ),
                    );
                } else {
                    console.error(
    ...
    if (defaultSocialSetArg) {
        ...
        try {
            await apiRequest(
                'GET',
                `/social-sets/${defaultSocialSetArg}`,
                null,
                {
                    exitOnError: false,
                },
            ...
            error(
                `Social set ${defaultSocialSetArg} not found or not accessible`,
            );
        }
        if (origKey) {
            process.env.TYPEFULLY_API_KEY = origKey;
        ...
        try {
            const origKey = process.env.TYPEFULLY_API_KEY;
            process.env.TYPEFULLY_API_KEY = apiKey;
            socialSets = await apiRequest(
                'GET',
                '/social-sets?limit=50',
                null,
                {
                    exitOnError: false,
                },
            ...
            console.error(
                fmt.warn(`Could not fetch social sets: ${err.message}`),
            );
            console.error(
                fmt.dim(
        ...
        if (socialSets) {
            if (!socialSets.results || socialSets.results.length === 0) {
                ...
                console.error(
                    fmt.dim(
                        'To get started, connect a social account at typefully.com:',
                    ),
                );
                console.error(
                    fmt.info(
                        `${fmt.link('https://typefully.com/?settings=accounts')}`,
                    ),
                );
                console.error('');
                console.error(
                    fmt.dim(
                        'After connecting, run: typefully.js config:set-default',
                    ),
                );
                ...
                const formatted = formatSocialSetsForDisplay(
                    socialSets.results,
                );
...
async function cmdConfigSetDefault(args) {
    ...
    if (!socialSetId) {
        ...
        if (formatted.length === 1) {
            ...
            const choice = await prompt(
                `${colors.bold}Enter number:${colors.reset} `,
            );
            const choiceNum = parseInt(choice, 10);
...
async function cmdDraftsCreate(args) {
    ...
    if (parsed.all) {
        ...
        if (allPlatforms.length === 0) {
            error(
                'No connected platforms found. Connect a platform at typefully.com',
            );
        }
...
async function cmdDraftsUpdate(args) {
    ...
    if (shouldUpdatePosts) {
        ...
        if (
            ...
            !explicitPlatformList.includes('x')
        ) {
            error(
                '--quote-post-url is only supported for X posts. Include x in --platform or remove the quote flag.',
            );
        ...
        if (explicitPlatformList) {
            ...
            if (platformList.length === 0) {
                ...
                const defaultPlatform =
                    await getFirstConnectedPlatform(socialSetId);
                if (!defaultPlatform) {
                    error(
                        'No connected platforms found. Connect a platform at typefully.com or specify --platform',
                    );
...
async function cmdCreateDraftAlias(args) {
    ...
    if (Object.prototype.hasOwnProperty.call(parsed, 'file')) {
        forwarded.push(
            '--file',
            coerceFlagValueToString(parsed.file, '--file'),
        );
    } else {
        let text;
...
async function cmdMediaUpload(args) {
    ...
    output({
        ...
        status: 'processing',
        message:
            'Upload complete but still processing. Use media:status to check.',
